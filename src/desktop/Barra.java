...
public class Barra extends Actor{
	...
	public void demo(){
		...
		for(int i=0;i<Arkanoid.bolas.size();++i){
			...
			//this.setX(bichito.getX()-(getAncho()/2)+(pelota.getAncho()/2)+(error*positivo));
		}
	...
	public void disparar(){
		if(Arkanoid.disparos.size()<5){ // Si no ha llegado a los 5 disparos
			...
			disparo.setX(Arkanoid.jugador.getX() + (Arkanoid.jugador.getAncho()) - (disparo.getAncho())); // Y obtenemos la X del jugador para que lo dispare desde el medio de la barra
			Arkanoid.disparos.add(disparo); //A�adimos al arraylist de disparos el disparo
			Disparo disparo2 = new Disparo(stage);
			disparo.setY(Arkanoid.jugador.getY());
			disparo.setX(Arkanoid.jugador.getX() + (Arkanoid.jugador.getAncho() - (disparo.getAncho())));
			Arkanoid.disparos.add(disparo2);
		}
