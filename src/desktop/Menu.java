...
/* 
 * � Copyright 2015 Gonzalez Ezequiel & Martin Lorenzo
 * 
 * 
...
public class Menu {
	...
	public void paintMenu(){
		...
		if(modogeneral){
			 ...
			 * Cancion 1
			 */
			Marco sonidoUno = new Marco(juego);
			sonidoUno.setNombreSprite("menu/cancion1.gif");
			if(seleccion==2){
				sonidoUno.setNombreSprite("menu/Wcancion1.gif");
			}
			sonidoUno.setX(Stage.primeracolumna);
			sonidoUno.setY(Stage.menuSegundaFila);
			sonidoUno.paint(g);
			/*
			 * Cancion 2
			 */
			Marco sonidoDos = new Marco(juego);
			sonidoDos.setNombreSprite("menu/cancion2.gif");
			
			if(seleccion==3){
				sonidoDos.setNombreSprite("menu/Wcancion2.gif");
			}
			sonidoDos.setX(Stage.primeracolumna);
			sonidoDos.setY(Stage.menuTerceraFila);
			sonidoDos.paint(g);
			...
			if(musicaactiva)
				puntosonido.setY(sonidoUno.getY());
			
			else
				
			puntosonido.setY(sonidoDos.getY());
		...
		if(modogeneral){
			...
			if(seleccion==2){
				...
				if(preguntarnombre){
					 ...
					 juego.nombre = JOptionPane.showInputDialog("Inserte su nombre para empezar a jugar: ");
		String aux = juego.nombre;
			if (esvip()){
				
				
				tatoe(); //funcion para elegir los niveles
			 ...
			 */
			if(seleccion==2){ //cancion 1

				//System.out.println("Ejecutar�a sonido");//Ejecutar sonido
				if(!Menu.musicaactiva){
					try{
						
						// aca debe ir el temita del select song
						Arkanoid.cancion1.start(); //Aplicamos sonido general
					}catch(Exception e){
			...
			}else if(seleccion==3){ //cancion 2

				/*System.out.println("Deber�a parase el sonido");//Paramos sonido
				Arkanoid.cancion1.parar(); //De momento no funciona
				musicaactiva=false;
				*/
				//System.out.println("Ejecutar�a sonido");//Ejecutar sonido
				if(!Menu.musicaactiva){
					try{
						...
						// aca debe ir el temita del select song
						Arkanoid.cancion2.start(); //Aplicamos sonido general
					}catch(Exception e){
					...
					musicaactiva=true;
						...
						if(!Menu.musicaactiva){
							try{
								...
								// aca debe ir el temita del select song
								Arkanoid.cancion3.start(); //Aplicamos sonido general
							}catch(Exception e){
	...
	public int tatoe(){
		juego.nivelinicial = Integer.parseInt(JOptionPane.showInputDialog("Seleccione Nivel a Jugar: \n1)Nivel 1 \n2)Nivel 2 \n3)Nivel 3 \n4)Nivel 4 \n5)Nivel5 "
				+ "\n6)Nivel 6 \n7)Nivel7 \n8)Nivel 8 \n9)Nivel9 \n10)Nivel 10"));
			
		switch (juego.nivelinicial){
	
		case 1:
		juego.nivelinicial = 1;
		break;
		case 2:
			juego.nivelinicial = 2;
			break;
		case 3:
			juego.nivelinicial = 3;
			break;
		case 4:
			juego.nivelinicial = 4;
			break;
		case 5:
			juego.nivelinicial = 5;
			break;
		case 6:
			juego.nivelinicial = 6;
			break;
		case 7:
			juego.nivelinicial = 7;
			break;
		case 8:
			juego.nivelinicial = 8;
			break;
		case 9:
			juego.nivelinicial = 9;
			break;
		case 10:
			juego.nivelinicial = 10;
			break;
		
	}
		return juego.nivelinicial;
}
